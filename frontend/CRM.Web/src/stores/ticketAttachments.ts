...
export const useTicketAttachmentsStore = defineStore('ticketAttachments', () => {
  const items = ref<TicketAttachment[]>([])
  ...
  async function fetchAttachments(ticketId: string) {
    ...
    try {
      items.value = await listTicketAttachments(ticketId)
    } catch {
  ...
  async function upload(ticketId: string, file: File) {
    if (uploading.value) {
      return
    }
  ...
  async function remove(ticketId: string, attachmentId: string) {
  ...
  async function download(ticketId: string, attachment: TicketAttachment) {
    error.value = null
